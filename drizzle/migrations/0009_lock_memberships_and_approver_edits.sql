DROP POLICY IF EXISTS "members insert self or admin" ON public.org_members;
  ...
  IF caller_role = 'approver' THEN
    IF NEW.status IS DISTINCT FROM OLD.status THEN
      ...
      IF NEW.title IS DISTINCT FROM OLD.title
        ...
        OR NEW.embed IS DISTINCT FROM OLD.embed
        ...
        OR NEW.use_embed IS DISTINCT FROM OLD.use_embed
    ...
    IF OLD.created_by IS DISTINCT FROM caller_id THEN
      RAISE EXCEPTION 'Approvers can only edit their own posts';
    END IF;
    RETURN NEW;
  END IF;
