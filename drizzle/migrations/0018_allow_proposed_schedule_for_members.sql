...
BEGIN
  ...
  IF caller_role NOT IN ('super_admin', 'admin') THEN
    IF NEW.status NOT IN ('draft'::public.post_status, 'pending'::public.post_status) THEN
      RAISE EXCEPTION 'New posts must begin as drafts';
    END IF;
    -- scheduled_at is only a proposed time for non-admins; publishing still requires approval
...
BEGIN
  ...
  -- Allow an idempotent re-save of a pending post (same status, no publishing fields touched)
